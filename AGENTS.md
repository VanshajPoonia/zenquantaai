...
- Custom assistant builder v1 is private and text-only. Custom assistants are user-owned Neon records layered over one built-in text mode (`general`, `creative`, `logic`, `code`, or `live`) with bounded extra instructions/default settings; they are not a new `AIMode`, do not replace Nova/Velora/Axiom/Forge/Pulse/Prism, and must not bypass plan/model limits.
