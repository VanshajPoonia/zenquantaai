...
create table if not exists public.zen_files (
  ...
  conversation_id text references public.zen_conversations(id) on delete set null,
  message_id text,
  provider text not null default 'supabase' check (provider in ('supabase', 'external', 'local')),
  bucket text,
  storage_path text,
  public_url text,
...
create table if not exists public.zen_generated_images (
  ...
  prompt text not null,
  negative_prompt text,
  storage_provider text check (storage_provider is null or storage_provider in ('supabase', 'external', 'local')),
  storage_bucket text,
  storage_path text,
  source_url text,
  width integer,
