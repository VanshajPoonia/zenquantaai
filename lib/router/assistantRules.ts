...
const CURRENT_INFO_PATTERN =
  /\b(latest|today|current|recent|recently|news|market update|what happened|breaking|this week|right now|up to date|current events|live update|as of today|as of now|web search|search the web|browse|look this up|find sources|cite sources|source-backed|verify this|fact check|current state|current landscape)\b/i
const CURRENT_RESEARCH_PATTERN =
  /\b(research the latest|look up current|what's new in|recent launch|recent update|market news|industry update|current status of|latest reports? on|latest data on|recent filings?|official sources? for)\b/i
