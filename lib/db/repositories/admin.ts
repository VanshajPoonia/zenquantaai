  ...
  async getOverview(): Promise<AdminOverview> {
    ...
    const mostExpensiveUsers = subscriptions
      .map((subscription) => {
        ...
        const userImageUsage = imageEvents.filter(
          (event) => event.userId === subscription.userId
        )
        const rawCostUsd =
    ...
    return {
      activeUsers: subscriptions.filter(
      ...
      usersByTier: {
        ...
        pro: subscriptions.filter((subscription) => subscription.tier === 'pro')
          .length,
        ultra: subscriptions.filter(
          (subscription) => subscription.tier === 'ultra'
        ).length,
        prime: subscriptions.filter(
          (subscription) => subscription.tier === 'prime'
        ).length,
      },
      pendingPlanRequests: requests.filter(
