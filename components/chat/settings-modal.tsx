...
function ModeSelectionCard({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group relative min-h-[152px] rounded-3xl border p-4 text-left transition-all duration-200 sm:min-h-[168px] sm:p-5 ${
        active
    ...
    >
      ...
      <div className="relative flex h-full flex-col gap-4 sm:gap-5">
        ...
        <div className="space-y-2">
          <p className="text-base font-semibold leading-tight text-foreground sm:text-lg">{config.name}</p>
...
export function SettingsModal({ open, onOpenChange }: SettingsModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[calc(100vw-1rem)] max-h-[92vh] max-w-[1120px] overflow-hidden rounded-[24px] border border-border/70 bg-background/95 p-0 shadow-2xl shadow-black/40 sm:w-[min(1120px,calc(100vw-2rem))] sm:rounded-[28px]">
        ...
        <DialogHeader className="border-b border-border/70 bg-gradient-to-b from-card/80 to-background px-4 py-5 text-left sm:px-6 sm:py-6 lg:px-8 lg:py-7">
          <DialogTitle className="text-2xl tracking-tight sm:text-3xl">Settings</DialogTitle>
          <DialogDescription className="max-w-2xl text-sm leading-6 sm:text-base sm:leading-7">
        ...
        <Tabs defaultValue="general" className="flex min-h-0 flex-1 flex-col gap-4 px-4 py-4 sm:gap-5 sm:px-6 sm:py-5 lg:gap-6 lg:px-8 lg:py-6">
          ...
          <TabsContent value="general" className="min-h-0 flex-1 space-y-6 overflow-y-auto pr-1 pb-6 sm:space-y-8 sm:pr-2">
            <div className="space-y-3">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                <div>
                  <h3 className="text-lg font-semibold sm:text-xl">Default Mode</h3>
